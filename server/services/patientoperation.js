    ...
    var returnstatus = new Promise((resolve,reject)=>{
        if(updateparams === undefined || referenceid === undefined)
        {
            ...
            resolve(
                ...
                storedb.hospitaldb.get(referenceid,function(err,doc){
                    var patientdata = [];
                    ...
                    storedb.hospitaldb.insert({doc,_rev:revision},referenceid,function(err,body){
                        if(!err)
                        {
...
module.exports = {getbookrequest,bookappointment,availability};